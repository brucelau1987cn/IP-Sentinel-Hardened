...
if [ -f "$CONFIG_FILE" ]; then
    ...
    REGION_NAME=$(read_config_val REGION_NAME)
    BASE_LAT=$(read_config_val BASE_LAT)
    BASE_LON=$(read_config_val BASE_LON)
    LANG_PARAMS=$(read_config_val LANG_PARAMS)
    VALID_URL_SUFFIX=$(read_config_val VALID_URL_SUFFIX)
    ENABLE_GOOGLE=$(read_config_val ENABLE_GOOGLE)
    ENABLE_TRUST=$(read_config_val ENABLE_TRUST)
    ...
    INSTALL_DIR=$(read_config_val INSTALL_DIR)
    ...
    LOG_FILE=$(read_config_val LOG_FILE)
else
    echo "配置文件丢失！退出执行。"
    exit 1
fi
...
if [ "$TOTAL_UA" -gt 0 ]; then
    ...
    IDX1=$(( SEED % TOTAL_UA ))
    IDX2=$(( (SEED * 127) % TOTAL_UA ))
    IDX3=$(( (SEED * 311) % TOTAL_UA ))
