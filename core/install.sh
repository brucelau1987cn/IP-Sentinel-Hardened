...
echo -e "\n[2/7] 正在连线云端，拉取全球节点地图..."
MAP_RETRY=0
while [ $MAP_RETRY -lt 3 ]; do
    curl -sL --retry 2 --retry-delay 2 "${REPO_RAW_URL}/data/map.json" -o "/tmp/map.json"
    if [ -s "/tmp/map.json" ]; then
        break
    fi
    ((MAP_RETRY++))
    echo "⏳ 拉取失败，第 $MAP_RETRY 次重试..."
    sleep 2
done
...
# ==========================================================
# 安全读取配置文件 (防止 source 注入, 替代直接 source)
read_config_val() {
    grep "^${1}=" "$CONFIG_FILE" 2>/dev/null | cut -d'=' -f2 | tr -d '"' | tr -d "'"
}
...
if [ "$SILENT_OTA" == "true" ]; then
    ...
    KEEP_LOGS="true"
    # 安全读取配置 (替代 source)
    REGION_CODE=$(read_config_val REGION_CODE)
    NODE_NAME=$(read_config_val NODE_NAME)
    NODE_ALIAS=$(read_config_val NODE_ALIAS)
    PUBLIC_IP=$(read_config_val PUBLIC_IP)
    BIND_IP=$(read_config_val BIND_IP)
    IP_PREF=$(read_config_val IP_PREF)
    TG_TOKEN=$(read_config_val TG_TOKEN)
    TG_API_URL=$(read_config_val TG_API_URL)
    CHAT_ID=$(read_config_val CHAT_ID)
    AGENT_PORT=$(read_config_val AGENT_PORT)
    ENABLE_OTA=$(read_config_val ENABLE_OTA)
    AGENT_VERSION=$(read_config_val AGENT_VERSION)
    SAFE_PUBLIC_IP=$(read_config_val PUBLIC_IP)
else
    echo -e "\n请选择操作:"
...
# 1. 优雅停止进程：先 SIGTERM 等3秒，再 SIGKILL 兜底
graceful_kill() {
    local pattern="$1"
...
graceful_kill "webhook.py"
graceful_kill "agent_daemon.sh"
graceful_kill "runner.sh"
...
# 部署日志轮转配置 (防止 sentinel.log 无限膨胀)
if [ -f "${INSTALL_DIR}/core/logrotate.conf" ]; then
    cp "${INSTALL_DIR}/core/logrotate.conf" /etc/logrotate.d/ip-sentinel 2>/dev/null || true
    echo -e "\033[32m✅ 日志轮转配置已部署 (每日轮转，保留7天)。\033[0m"
fi
